...
void getselect(int n) {/*判断为求first或follow集合*/	//在这里调用了first()函数
	for (int i = 0; i < n; i++) {
		int x1 = tableprep(row, c[i].vn);
		first(x1,i,i);		//在first()函数中调用了follow()函数
	}
	return;
}
...
int main() {/*主函数，用于读入词法分析完成5得到的单词序列*/		//这是整个编译器的主函数调用了包括词法分析在内的所有函数
	...
	init_twoele(TOKEN);		//用于初始化二元式，为读取wordsequence.txt文件作准备

	/*下面这个部分主要是读取wordsequence.txt的内容
		并将读取到的内容写入TOKEN结构体*/
	...
	cout << "产生式如下" << endl;	//读出了产生式文件grammar.txt中的产生式
	for (int i=0;;i++) {
		gram >> pro[i];
		if (strcmp(pro[i], "#") == 0) {
			break;
	...
	gra(alystack);
	brief();
	char ac[] = "action.dat";	//这个是语法分析部分的输出
	...
	getchar();
	analysistable();	//调用了符号表中的analysistable()函数，开始构建分析表，从这里进入符号表.cpp
	return 0;
}
